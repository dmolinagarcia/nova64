...
| Trampoline | The short privileged stubs in bank `$00` that receive every vector — the CPU forces PBR=0 on entry — and `JSL` straight into the real kernel in `$01` (→ [L.11](sec_ai_l#l11)). |
...
| Term | Meaning |
|---|---|
| Bank | A 64 KB slice of the 65816's 24-bit space, selected by DBR/PBR. The virtual bank map is in [L.10](sec_ai_l#l10). The map is fixed: `$00` stack, direct page and vectors · `$01` kernel · `$02`–`$FD` user · `$FE` VRAM window · `$FF` privileged I/O. |
