...
The document's terminology in one place. Every acronym is also expanded on first use where it appears, so this sheet is deliberately redundant — it exists to be jumped into, not read through. It is grouped by domain, and within each group runs roughly in the order the concepts appear. The figures have a sheet of their own ([Z2](sec_ai_z2)).

## Virtual memory and the MMU
  NOTE: → [sheet K](sec_ai_k) · [sheet L](sec_ai_l)
...
| Physical address | The address that reaches the SRAM/SDRAM chips. 27 bits here. 27 bits here — a 128 MB ceiling, with the SDRAM frames at the bottom of that space and the pinned SRAM frames at the top ([L.6](sec_ai_l#l6)). |
...
| Trampoline | The short privileged stubs in bank `$00` that receive every vector — the CPU forces PBR=0 on entry — and `JSL` straight into the real kernel in `$FD` (→ [L.11](sec_ai_l#l11)). |
| ASID recycling | Reassigning a used ASID to a new process. Legal only after flushing that ASID's TLB entries; skipping the flush leaks the old address space into the new one, silently (→ [M.8](sec_ai_m#m8)). |
| Address as opcode | Command convention where each operation has its own address and the value written is its argument, not a command code (→ [M.2](sec_ai_m#m2)). |
...
| Watchdog | Timer that resumes PHI2 and raises NMI or ABORT if a fill overruns — without it a hung fill freezes the machine with no clock and no diagnostic path. It covers Neon's read stall too, which is what makes an unfitted Neon a fault report rather than a dead board ([F.11](sec_ai_f#f11)). |
...
## CPU, bus and gateware
  NOTE: → [sheet B](sec_ai_b) · [sheet F](sec_ai_f)

| Term | Meaning |
|---|---|
| Bank | A 64 KB slice of the 65816's 24-bit space, selected by DBR/PBR. The virtual bank map is in [L.10](sec_ai_l#l10). The map is fixed: `$00` stack, direct page and vectors · `$01` VM interpreter · `$02`–`$EF` user · `$F0`–`$FD` kernel, growing down from `$FD` · `$FE` VRAM window · `$FF` privileged I/O. |
...
| BE | Bus Enable — the pin that makes the physical CPU tristate its address, data and RWB drivers and release the bus. Driven by Helium, which uses it far more often than Argon would (→ [R.3](sec_ai_r#r3)). |
| RESB | The 65816's active-low reset. Released by the RP2354B as the last step before the CPU runs. Pulled down when no bitstream is loaded, so a blank board holds the CPU in reset instead of running it into an undriven bus. |
...
| RDY | Bidirectional, open-drain: Helium pulls it low to freeze the CPU while it takes the shared nets ([F.4](sec_ai_f#f4)), and the CPU pulls it low itself during `WAI`. **Cache-miss stalls do not use it** — [D16](sec_ai_q#d16) moved those to PHI2 gating, since some W65C816S revisions ignore RDY during write cycles. |
...
| Softcore | A CPU implemented as gateware rather than as a chip. The footprint on Argon ships unpopulated, and what a core owes the board is a pin list as much as an instruction set ([E.13](sec_ai_e#e13)). **Which core it would be has changed**: [D88](sec_ai_q#d88) makes it bytecode-native rather than a 65816 (→ [sheet VM3](sec_ai_vm3), [sheet E](sec_ai_e)). |
...
| Cycle-exact | A core reproducing the original's timing cycle for cycle. Needed to emulate a specific machine; unnecessary here, where only programmer-visible behaviour must match (→ [E.6](sec_ai_e#e6)). |
...
| HDL | Hardware Description Language — Verilog or VHDL. noVa64 gateware is Verilog-2005 ([D101](sec_ai_q#d101)). |
...
| Expansion slot | The parallel connector of [sheet X](sec_ai_x): the machine's own bus, buffered, with a card as a third tenant of the contract Helium already has with Neon. Unpopulated in the first build. |
| `/EXP_SEL` · `/EXP_BSY` · `/EXP_IRQ` | The slot's three nets — select, stall, interrupt — mirroring `NEON_BUS_SEL`/`BSY`/`IRQ`, with `/EXP_BSY` idling **not busy** because an empty slot must not stall the machine ([X.11](sec_ai_x#x11)). |
| Slot aperture | The 1 MB per slot the MMU can map into a process, carved out of the frame numbers [F.10](sec_ai_f#f10) leaves unpopulated and addressed on A11–A19, which are idle while the SRAM's `/CE` is ([X.8](sec_ai_x#x8)). Always non-cacheable. |
| Register window vs aperture | The two ways a card is reachable, and the split is [T1.21](sec_ai_t1#t121)'s: **control** in bank `$FF`, unmappable to user space by construction; **bulk** through the aperture, mappable with permissions in front of it. |
| Geographic addressing | Telling a card which slot it is in with connector pins strapped differently in each position, so the same board works in either one with no jumper ([X.14](sec_ai_x#x14)). |
| Bus master | A device that drives the address bus instead of the CPU. Here: Helium, and nothing else — a card cannot honour the cache-coherence discipline of [M.12](sec_ai_m#m12), so [D48](sec_ai_q#d48) refuses it the bus rather than trusting it ([X.16](sec_ai_x#x16)). |
| `I2C-EXP` | The third I2C bus, Helium's own and the only one where Helium is master. The slow expansion tier: devices mirrored into I/O windows rather than reached transactionally ([X.22](sec_ai_x#x22)). |
...
| LDO | Low-DropOut regulator — a linear regulator, which wastes the voltage it drops as heat. The 1.2V rail used to be one and is now a buck ([D20](sec_ai_q#d20)). |
...
| OTG | A charger mode in which the device sources 5V onto its own VBUS. Present on the BQ25896 and deliberately unused ([D21](sec_ai_q#d21)). |
...
| The link | The one SPI channel between the EC and Helium, behind `LINK_CSN` — **the whole of the data plane between the two** ([sheet D3](sec_ai_d3)). Every block inside Helium that the EC talks to is an endpoint on it, addressed by a header byte rather than by its own chip select ([D80](sec_ai_q#d80)). |
| Mailbox | What the link used to be called when it was only HID and system traffic. **It is now two endpoints on the link — `0x03` and `0x04` — at the same level as the Debug Agent, not the link itself** ([D3.9](sec_ai_d3#d39)). |
| `LINK_CSN` | The link's chip select, formerly `MBX_CSN`. The rename is the visible face of the merge: `DBG_CSN` is withdrawn and one select serves every endpoint ([D3.9a](sec_ai_d3#d39a)). |
| Endpoint | A destination inside Helium named by the first byte of a link frame. Each has a class — sink, source or duplex — a commit discipline, and **its own inbound queue, which is what keeps a full queue from blocking anyone else** ([D3.12](sec_ai_d3#d312), [D83](sec_ai_q#d83)). |
| Router | The block behind the SPI front end that reads the header and dispatches the frame to an endpoint. **It dispatches at header completion into a per-endpoint sink rather than buffering the payload centrally**, and it sits in a reset domain no endpoint can assert ([D3.12a](sec_ai_d3#d312a), [D85](sec_ai_q#d85)). |
| Frame | One assertion of `LINK_CSN`: three header bytes each way, payload, then a frame CRC — `4 + LEN` bytes, 259 at most. **The maximum is a latency bound, not a limitation**: it caps the delay one endpoint can impose on another at about 207 µs ([D3.11](sec_ai_d3#d311), [D84](sec_ai_q#d84)). |
| `HCRC` / `FCRC` | The link's two CRC-8s. **`HCRC` protects the routing decision**, which is the catastrophic failure — a corrupted endpoint byte turns a keystroke into a halt or a memory write. **`FCRC` covers the whole frame and decides whether it is committed** ([D3.11b](sec_ai_d3#d311b)). |
| `EPSTAT` | Inbound free space, in bytes, for the endpoint a frame is addressed to — the descendant of the mailbox's four-bit `IN_SPACE`. It is on the wire **before the EC sends any payload**, so a frame to a full endpoint can be abandoned for three bytes rather than rejected after 259. **It may under-report and must never over-report** ([D3.11a](sec_ai_d3#d311a)). |
| Buffered / streaming commit | Whether an endpoint stages a frame and commits it on `ACK`, or commits as bytes arrive. **Streaming is permitted only where retransmitting a rejected frame verbatim is harmless**, which is why [R.25](sec_ai_r#r25)'s bulk write carries an absolute address in every frame ([D3.11c](sec_ai_d3#d311c)). |
| `WRITE_BURST` | The Debug Agent's bulk memory write — opcode, 24-bit absolute address, up to 251 data bytes. **It is how `bios.bin` reaches SRAM**, which makes the debug write path boot-critical and exercised on every power-up ([R.25](sec_ai_r#r25), [D82](sec_ai_q#d82)). |
| INV-1 | The invariant that the Debug Agent's request port has exactly one source and nothing reachable from the 65816 address space can drive it. **A connectivity property, verified structurally** — REV B credited it to a chip select, which never delivered it ([R.26](sec_ai_r#r26), [D81](sec_ai_q#d81)). |
| Head-of-line blocking | A queue in which one stalled item holds up everything behind it. **The failure the per-endpoint sinks exist to prevent**: a CPU that stops draining keystrokes must not stall the debug traffic sent to find out why ([D3.12a](sec_ai_d3#d312a)). |
...
| `CFG_SS` parking | Holding each configuration select **low** for the whole session once its `CDONE` has risen. The iCE40 samples that line at reset exit to choose slave or master configuration mode, so a line idling high brings the part up **driving `SCK` against the EC** after any accidental reset ([D3.8](sec_ai_d3#d38), [D86](sec_ai_q#d86)). |
...
## Video, audio and peripherals
  NOTE: → [sheet H](sec_ai_h) · [sheet T1](sec_ai_t1) · [sheet T3](sec_ai_t3) · [sheet W](sec_ai_w)
...
| NEON_BUS_SEL | Helium → Neon: "this cycle is validated for you." Qualifies **both** of Neon's windows — `$FE` after translation and the permission check, `$FF:8000`–`$FF:80FF` after the bank decode and the privilege check ([B.6](sec_ai_b#b6)). |
| NEON_BUS_BSY | Neon → Helium: "I cannot serve this cycle yet." **Busy by default**, so an absent or unconfigured Neon stalls the machine into a watchdog report rather than onto an undriven bus. Its meaningful edge is the deassertion, which means the data is ready ([T1.18](sec_ai_t1#t118)). |
| Bus-busy stall | How the CPU reads VRAM: Helium decodes the read, holds PHI2 **low** unconditionally, and raises it when Neon clears busy. Stall first, ask afterwards — which removes the race a wait line asserted against the rising edge would have had ([D37](sec_ai_q#d37)). |
| Aperture | The same opening, named from Neon's side. **Data only** — Neon's control registers are not in it but at `$FF:8000`, because `$FE` is user-mappable and `$FF` is not ([D36](sec_ai_q#d36)). |
| Px-doubling | Drawing at a lower resolution and emitting each pixel more than once in both axes to fill 1024×600. Not a policy here but a mode: **Mode 2b** at 2×2, and Mode 2a at 3×3 ([D35](sec_ai_q#d35)). |
...
| Channel | One of the blitter's four memory ports under the model of [sheet T3](sec_ai_t3) — A, B, C read, D writes. Each carries a pointer, a signed modulo and an enable; a disabled read channel supplies a constant and costs no bandwidth. |
...
| Backing store | A surface holding one window's complete contents, **including the parts currently hidden behind other windows** — exactly what a flat framebuffer does not keep ([Q60](sec_ai_q#q60)). |
| Compositor | The kernel code that assembles the visible framebuffer from the backing stores, driving the blitter. Distinct from the window manager, which decides what should be where. Which model it runs is still open: repaint every window intersecting the damage, or recompose from backing stores ([Q60](sec_ai_q#q60)). |
...
| Damage · dirty rectangle | A region of the screen whose composited content is no longer valid and must be rebuilt. Under [T3.31](sec_ai_t3#t331) a bug here costs performance, not corruption, because full recomposition stays correct. |
| Cookie-cut vs colour key | The two ways to make a sprite non-rectangular. A key designates one palette index transparent and needs no extra memory; a mask is a second bitmap and needs generating and keeping in step. The blitter model decides which is available ([Q58](sec_ai_q#q58)). |
...
| Display list patching | Writing into a list while Neon executes it — the intended usage pattern, not an abuse. Moving an object is one 32-bit store to one word ([T1.41](sec_ai_t1#t141)). |
...
| SDRAM bank | One of four independent arrays inside the device; accesses to different banks interleave with no row-activation penalty. A layout convention for copies, not something hardware enforces ([Q50](sec_ai_q#q50)). |
| FPC | Flexible Printed Circuit — the flat ribbon connecting the panel. "FPC-50" is its 50-contact connector. |
| eDP | Embedded DisplayPort — serial panel interface. Rejected for v1 as it needs a bridge chip ([D05](sec_ai_q#d05)), and **closed outright** by [D70](sec_ai_q#d70): the bridge category is BGA-only and its one hand-solderable member has no distribution. |
| LVDS | Low-Voltage Differential Signalling — an electrical technique, ~350 mV about a 1.2 V common mode into a 100 Ω-terminated pair. Not a protocol by itself ([U1.4](sec_ai_u1#u14)). |
| FPD-Link · OpenLDI | The protocol layered on LVDS for panels: 7:1 serialization of parallel RGB onto three or four data pairs plus a clock pair. **The clock pair carries the pixel clock, not the bit clock.** OpenLDI is the same scheme under the other name. |
| Serializer | The part turning parallel RGB into LVDS pairs — here two SN75LVDS83B in TSSOP, fed from one shared bus so a dual-channel panel gets two identical in-phase streams ([U1.16](sec_ai_u1#u116), [U1.19](sec_ai_u1#u119)). |
| VESA · JEIDA mapping | The two incompatible schemes for distributing a component's bits across the seven-bit lanes. Choosing wrong gives **a tinted, posterised image rather than a black screen**, which is why it is a datasheet item and not a bring-up guess ([U1.27](sec_ai_u1#u127)). |
| Even/odd split | How a dual-channel panel divides pixels between its two channels. Interleaved by pixel is standard and is what lets both channels share one bus under horizontal duplication; **split by screen half, the technique fails and the panel is unusable** ([U1.17](sec_ai_u1#u117)). |
...
| 44,146 Hz | This machine's actual sample rate — 103.125 MHz / (32 × 73). Not 44,100, because the iCE40 PLL is integer-only and 44.1 kHz needs a denominator factor of 441 that no setting can produce. +1.8 cents of pitch, published in `AUD_RATE` so nothing has to assume it ([W.11](sec_ai_w#w11), [Q77](sec_ai_q#q77)). |
| `XSMT` | The DAC's soft-mute pin, held low through an RC while the rail rises — **the anti-pop**. It protects power-up only; the power-down click is [Q76](sec_ai_q#q76). |
| Saturating add | Clamping a sum at full scale instead of letting it wrap. Four channels at full volume exceed full scale, and a wrap turns the loudest moment in the material into full-amplitude noise ([W.27](sec_ai_w#w27)). |
...
| Term | Meaning |
|---|---|
| BIOS | Here it means two things kept distinct: the RP2354B is the "board BIOS" (everything pre-CPU), and `BIOS.BIN` is the "system BIOS" (everything post-reset). |
...
| devfs | The synthetic filesystem where those `/dev/*` nodes would live — no bytes on the SD card. **Not yet**: `open` resolves `/dev/` by prefix against the device table until something needs to list devices ([D108](sec_ai_q#d108)). |
...
| tty | A terminal device: a line discipline between an output backend and an input backend. The console is `/dev/tty` — Mode 0 out, Helium's HID queue in — and a serial line, where there is one, is `/dev/ttyS0` ([CN1.9](sec_ai_cn1#cn19)). |
| Line discipline | The layer of a tty that edits a line before any program sees it — echo, erase, kill line — and turns Ctrl-C into an interrupt. It lives in the kernel, not in the shell ([D104](sec_ai_q#d104)). |
| Canonical mode | The line discipline's default: `read()` returns complete lines. **Raw mode** passes every byte at once, with no echo and no editing. |
| Monolithic build | The console linked as one image, with no MMU and no processes, the shell calling kernel functions directly. The **user-mode build** relinks the same shell against the `COP` stubs and runs it as PID 1 ([CN1.1](sec_ai_cn1#cn11)). |
| Degraded mode | The console with no root filesystem: a prompt showing `(no root)`, and only the built-ins that need no files ([CN1.7](sec_ai_cn1#cn17)). |
| Boot partition | The card partition holding the kernel image, which the BIOS reads with no filesystem code at all ([D107](sec_ai_q#d107)). |
| Typematic repeat | A held key repeating. Generated by the EC, since USB HID keyboards do not repeat on their own ([D106](sec_ai_q#d106)). |
| CON-00–CON-04 | The console's series, from a kernel that prints its banner to a shell running as PID 1 ([sheet CN1](sec_ai_cn1)). |
...
| Term | Meaning |
|---|---|
| VFS | Virtual File System — the layer between the syscall table and the filesystem drivers. It owns path resolution, the mount table, vnode identity and the error space, because none of those can be decided inside a driver that cannot see across a mount boundary ([Y1.1](sec_ai_y1#y11)). |
| vnode | The in-memory object for one *in-use file*, cached and reference-counted, identified by its mount and a stable file identifier. Not the same as an open file: two `open()` calls share a vnode and hold separate offsets ([Y1.5](sec_ai_y1#y15)). |
| Inode | The on-disk record of a file — type, permissions, size, times, and where its data is. 128 bytes in both NVFS and the ext2 configuration used here. A name is not part of it: names live in directories and point at inode numbers, which is what makes hard links possible. |
| Superblock | The block that describes the whole volume: sizes, region locations, counters, feature flags. NVFS keeps a byte-identical backup at the last block, written only by `mkfs` and `nvfsck`, and **never silently falls back to it** ([Y2.6](sec_ai_y2#y26)). |
| Bitmap · summary | One bit per block or inode saying whether it is allocated, plus — in NVFS — a free-count per bitmap block so the allocator can skip a full one without reading it ([Y2.7](sec_ai_y2#y27)). |
| Extent | A run of consecutive blocks described by one entry: first logical block, first physical block, length. Replaces the indirect tree for files laid down in one or a few runs, which is most of them ([Y2.10](sec_ai_y2#y210)). |
| Indirect block | ext2's alternative: a block full of block pointers, with double and triple levels above it. Cheap to implement, expensive to read — each level is another transfer before any data moves ([Y3.5](sec_ai_y3#y35)). |
| Hole | A logical block covered by no extent and no pointer. Reads as zeros and occupies nothing. |
| Journal | A log of metadata changes written before the changes reach their home locations, so that recovery is a forward scan and a sequence of block copies. NVFS's is **physical, redo-only, metadata-only and circular**, which is what makes replay idempotent ([Y2.15](sec_ai_y2#y215)). |
| Redo-only · idempotent | The journal holds new contents, never old ones, so replay copies blocks to fixed destinations — running it twice is the same as running it once, and a crash *during* recovery is harmless. |
| Checkpoint | Writing a committed transaction's blocks to their home locations and advancing the journal's start pointer. May be deferred and batched; **advancing it before those writes are durable is the one way to lose data** ([Y2.16](sec_ai_y2#y216)). |
| Barrier | The point at which every write issued before it is durable before any issued after. On SD that means waiting out the busy state and confirming — an assumption rather than a guarantee ([Q96](sec_ai_q#q96)). |
| `data=ordered` | The rule that a transaction's file data reaches the medium before its commit block, so metadata can never reference a block whose contents are stale. The faster alternative is refused here ([Y2.18](sec_ai_y2#y218)). |
| Orphan | An inode whose link count reached zero while it was still open. Threaded onto a list in the superblock and freed at the next mount, which is what stops a crash from leaking space ([Y2.19](sec_ai_y2#y219)). |
| Deferred delete | Unlinking a file that is still open: the name goes, the blocks are released when the last reference does. Makes deleting an open file safe rather than catastrophic ([Y1.18](sec_ai_y1#y118)). |
| Feature flags | Three masks in the superblock. An unknown *incompatible* bit refuses the mount, an unknown *read-only* bit mounts read-only, an unknown *compatible* bit is ignored — and the implementation **must name the bit it did not recognise** ([Y2.20](sec_ai_y2#y220), [Y3.4](sec_ai_y3#y34)). |
| htree · `DIR_INDEX` | ext2's hashed directory index, hidden inside fake entries so that a linear reader sees the directory correctly. **Reading is safe and writing is not**: an insert that does not update the index leaves a file that exists, occupies space, appears in `readdir` and cannot be opened by name ([Y3.6](sec_ai_y3#y36)). |
| `readdir` cookie | An opaque value a driver hands back to resume iteration. NVFS advances a slot index and ext2 a byte offset; neither is expressible in the other's terms, so the VFS stores it and never interprets it ([Y1.3](sec_ai_y1#y13)). |
...
## The windowing OS
  NOTE: → [sheet V](sec_ai_v)

| Term | Meaning |
|---|---|
| `wserver` | The window server: window objects, z-order, input routing, focus, decoration and client validation. A **user task** here, because none of that needs a privileged instruction — the compositor beside it is kernel code, because command emission does ([Q62](sec_ai_q#q62)). |
...
| Shared command buffer | The 4 KB page `mshare` maps writable in the client and readable in the server. Drawing ops accumulate in it and flush with one syscall — **a syscall per primitive would cost more than the primitive** ([V.20](sec_ai_v#v20)). |
...
| `DRAW_GLYPHS` | A proposed command taking a string pointer and a count, so a line of text is one command with no cache at all. **Complementary to the cache rather than a replacement**: the cache wins for stable labels, this wins for text that changes ([Q64](sec_ai_q#q64)). |
...
| Decoration | The frame, title bar and gadgets the server draws **into the window's own backing store**, above the content area and clipped away from the client. Composited as one wide blit rather than four narrow ones, which is what [T3.18](sec_ai_t3#t318) asks for. |
...
| Port · signal · message | The Exec model, proposed for this kernel: a port bound to one bit of a task's 32-bit signal mask, fixed 32-byte messages copied by the kernel, and `task_wait` as **the single blocking primitive** ([Q63](sec_ai_q#q63)). |
...
| Overdraw | Total composited area ÷ screen area, counting the desktop clear. The number that decides whether full recomposition fits: ~1.8× is comfortable at 1 bpp and consumes the whole frame at 8 ([V.10](sec_ai_v#v10)). |
...
| Term | Meaning |
|---|---|
| Debug agent | The block inside Helium that performs memory and bus accesses on command from the RP2354B. A requester in the arbiter, not an external master. |
...
| Scan chain | The register-readout path debuggable CPUs provide. The W65C816S has none, which is why its registers can only be inferred or dumped by a stub (→ [R.15](sec_ai_r#r15)). |
...
## The NVM32 virtual machine and noVa128
  NOTE: → [sheet VM1](sec_ai_vm1) · [sheet NV1](sec_ai_nv1)

| Term | Meaning |
|---|---|
| NVM32 | The project's 32-bit register virtual machine: sixteen 32-bit registers, a 24-bit address space, 16-bit code units and no flags. Designed so that **one binary runs interpreted on the 8 MHz 65816 and natively on a core** ([VM1.1](sec_ai_vm1#vm11)). Deferred until the kernel runs its first user process. |
| Bytecode | An instruction set executed by software, or by hardware built for it, rather than by the CPU the machine is made of. Here it is not an interchange format: the same NVX image is the input to every executor. |
| Architectural state | The state an instruction set promises to preserve — for NVM32, **{R0–R15, PC} and nothing else** ([VM1.14](sec_ai_vm1#vm114)). It is what makes a process migratable mid-execution between interpreter and core, and every proposal that would add to it pays against that. |
| Executor | Any implementation that runs NVM32: the C11 reference interpreter, the 65816 interpreter, the bytecode coprocessor, or Argon 2's native mode. All four must be **bit-identical**, down to shift counts ≥ 32 and division by zero. |
| Golden model | The implementation every other one is compared against. Here the C11 reference interpreter, hosted in the emulator ([EM1.3a](sec_ai_em1#em13a)) — the same differential pattern already used on the filesystem ([Y2.22](sec_ai_y2#y222)). |
| NVX | The NVM32 executable format: magic, ISA version, a feature-bit vector, a 24-bit entry PC and a section table. **Linked at fixed addresses and never relocated**, because every process owns its whole 24-bit space ([VM1.42](sec_ai_vm1#vm142)). |
| ILP32-24 | NVM32's C ABI: 32-bit `int`, `long` and pointer, the pointer holding a 24-bit address in four bytes with the top byte required to be zero. Deliberately divergent from Calypsi's 16-bit `int`, and the two meet only at the syscall marshalling layer ([VM1.35](sec_ai_vm1#vm135)). |
| 32-bit clean | A pointer model where the unused high bits **must** be zero and are checked rather than ignored — the 68000 lesson applied on purpose, since software that stored flags there broke when full addressing arrived ([VM1.16](sec_ai_vm1#vm116)). |
| Code unit | The 16-bit quantum an NVM32 instruction is built from: one, two or three of them, with **the opcode byte alone determining the length**. The Dalvik pattern, chosen because one 65816 fetch yields an opcode plus two register nibbles ([VM1.22](sec_ai_vm1#vm122)). |
| Dispatch | The cost of deciding which handler runs, paid once per VM instruction. ~18 of the ~70 cycles of an interpreted `ADD`, and **the number every encoding candidate is trying to amortise** ([VM1.45](sec_ai_vm1#vm145)). |
| Superinstruction | One opcode doing the work of a common sequence, so the sequence pays one dispatch instead of several. `ENTER`, `LEAVE` and compare-and-branch are the adopted ones ([VM1.28](sec_ai_vm1#vm128)). |
| The gate | The single indirect entry point through which native code enters bytecode, resolved at boot into the system information block. Three implementations — interpreter, coprocessor, native mode — behind one signature, **so only a 3-byte vector differs between machines** ([VM2.16](sec_ai_vm2#vm216)). |
| System information block | The BIOS-to-kernel contract: a shared read-only page in bank `$00` carrying a format version, the CPU identity, Helium's capabilities, coprocessor presence and the resolved gate ([I.7](sec_ai_i#i7), [D91](sec_ai_q#d91)). **BIOS implementations may differ between machines; this may not.** |
| `WDM` | Opcode `$42`, reserved by WDC for expansion and **a two-byte no-op on a real 65816**. Used as the CPU probe and as Argon 2's NVM32 mode gate — which bounds [M.3](sec_ai_m#m3) rather than reversing it ([D89](sec_ai_q#d89)). |
| Shadow dispatch bank | A second copy of the interpreter whose 256 dispatch entries all point at the exit handler. Switching a process's saved PBR lands it in the kernel at the next exact VM instruction boundary, **at zero cost on the common path** ([VM2.33](sec_ai_vm2#vm233)). |
| vbcc | The retargetable C compiler selected for NVM32 — ruled out for the 65816, where it has no 16-bit code generation, and chosen here because the target was designed after it ([D96](sec_ai_q#d96)). |
| T0–T5 | The NVM32 toolchain's gate series, independent of E/G/K. **T3 collects the opcode histogram and T4 freezes the ISA on it** ([VM4.31](sec_ai_vm4#vm431)). |
| noVa128 | The successor machine: Helium 2, Neon 2 and Argon 2, a 16-bit inter-FPGA path, and one core replacing the W65C816S ([sheet NV1](sec_ai_nv1)). |
| Argon 2 | noVa128's CPU: a tri-mode softcore running 6502 emulation, 65816 native and NVM32 native. **Not to be confused with Argon**, which in noVa64 is FPGA-C, the device holding the unpopulated slot ([NV1.1](sec_ai_nv1#nv11)). |
| Instruction-compatible | Argon 2's contract: architectural state matches a real 65816 after **every instruction**, while cycle counts, interrupt latency and internal bus accesses may diverge. Affordable only because the PHI2 stall already withdrew cycle counting as a measure of time ([NV1.5](sec_ai_nv1#nv15)). |
| Idempotent on read | A register whose value is unchanged by reading it. Required of every memory-mapped register, because a 65816 read-modify-write performs spurious accesses that an instruction-compatible core does not reproduce ([M.15](sec_ai_m#m15), → [Q170](sec_ai_q#q170)). |
