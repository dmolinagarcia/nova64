...
- EM1.3a — **A fourth purpose arrives with the VM programme, and it is the cheapest of the four: this emulator hosts the C11 reference interpreter for NVM32, which is the golden model for every executor** — the 65816 interpreter, the bytecode coprocessor and Argon 2 all validate against it ([VM1.4](sec_ai_vm1#vm14), [VM3.20](sec_ai_vm3#vm320)). It is Stage 1 of that programme, it is PC-side software competing for no hardware time, and **it retires the largest open risk in the ISA before any of it is built**: the opcode histogram it produces is what freezes the encoding ([VM4.34](sec_ai_vm4#vm434)).
  NOTE: It also widens this sheet's scope by less than it appears. **The softcore stays out of the emulator and in Verilator** ([EM1.5](sec_ai_em1#em15)), exactly as the boundary below already places it; what comes in is an interpreter for a 32-bit bytecode, which is ordinary host software and touches no subsystem modelled here.
...
- EM1.11 — **Co-simulation is not adopted** unless a defect class emerges that the emulator cannot isolate. It is unscheduled and needs no further document.
  NOTE: It is easier to revisit because the RTL is Verilog ([D101](sec_ai_q#d101)), chosen on toolchain grounds — Yosys and nextpnr are Verilog-native, VHDL needs the GHDL plugin path, and Verilator is Verilog-only. Not an emulator decision, recorded here because this is where the constraint was noticed.
