...
- EM6.19a — [[open]] **Debug port, proposed: a transport-independent GDB remote serial protocol stub**, carried over TCP in the native build and over WebSocket in the browser build. A browser page cannot open a listening port, so the browser build connects outward. **Status: proposal only.** See DN-PLAN-TRAINING-001's [TR3](sec_ai_tp1#tr3) and [TR4](sec_ai_tp1#tr4), on [sheet TP1](sec_ai_tp1), and the planned DN-SW-DEBUG-001 (→ [Q189](sec_ai_q#q189)).
