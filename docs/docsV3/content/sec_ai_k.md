...
Explanatory groundwork, not normative: it explains paging from first principles and gives the reasoning behind the field widths, so that the machinery in [sheet L](sec_ai_l) can be read without stopping to decode acronyms. Term-by-term definitions live in the [glossary, sheet Z1](sec_ai_z1). It introduces no design decision — where it disagrees with an architecture sheet, the architecture sheet wins.
