...
| Register window vs aperture | The two ways a card is reachable, and the split is [T.21](sec_ai_t#t21)'s: **control** in bank `$FF`, unmappable to user space by construction; **bulk** through the aperture, mappable with permissions in front of it. |
...
## Video, audio and peripherals
  NOTE: → [sheet H](sec_ai_h) · [sheet T](sec_ai_t) · [sheet U](sec_ai_u) · [sheet W](sec_ai_w)
...
| NEON_BUS_BSY | Neon → Helium: "I cannot serve this cycle yet." **Busy by default**, so an absent or unconfigured Neon stalls the machine into a watchdog report rather than onto an undriven bus. Its meaningful edge is the deassertion, which means the data is ready ([T.18](sec_ai_t#t18)). |
...
| Channel | One of the blitter's four memory ports under the model of [sheet U](sec_ai_u) — A, B, C read, D writes. Each carries a pointer, a signed modulo and an enable; a disabled read channel supplies a constant and costs no bandwidth. |
...
| Damage · dirty rectangle | A region of the screen whose composited content is no longer valid and must be rebuilt. Under [U.31](sec_ai_u#u31) a bug here costs performance, not corruption, because full recomposition stays correct. |
...
| Display list patching | Writing into a list while Neon executes it — the intended usage pattern, not an abuse. Moving an object is one 32-bit store to one word ([T.41](sec_ai_t#t41)). |
...
| eDP | Embedded DisplayPort — serial panel interface. Rejected for v1 as it needs a bridge chip ([D05](sec_ai_q#d05)). |
...
| Decoration | The frame, title bar and gadgets the server draws **into the window's own backing store**, above the content area and clipped away from the client. Composited as one wide blit rather than four narrow ones, which is what [U.18](sec_ai_u#u18) asks for. |
