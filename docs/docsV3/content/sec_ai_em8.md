...
- EM8.3 — **The adapter reaches the machine only through the Debug Agent's command set, plus host-only extensions marked as such** ([D127](sec_ai_q#d127)). That is [EM3.3](sec_ai_em3#em33)'s debug parity applied to a client: the adapter is written once and pointed at hardware later without a rewrite. The extensions are what only the emulator can offer — exact registers, any number of breakpoints, the shadow stack, the counters of [EM3.15](sec_ai_em3#em315) — and each is a separate command that the hardware backend answers with "not available" rather than with an imitation.
...
- EM8.8 — **There is one build, and it always emits debug information** ([D128](sec_ai_q#d128)). `ca65 -g` and `ld65 --dbgfile` leave every byte of the image unchanged, so a separate debug target — which the prior art had — buys nothing except a second binary able to drift from the first. The flags are in [O.10](sec_ai_o#o10).
- EM8.9 — **C is mapped through DWARF, and that path is not yet confirmed** (→ [Q192](sec_ai_q#q192)). Calypsi's objects are ELF with DWARF debug information, which a DWARF reader handles in principle. Whether it survives to the image the loader actually uses, and how the 24-bit addresses and the far pointers of [O.2](sec_ai_o#o2) are encoded in it, has not been checked. **Assembly comes first** because the boot images, the BIOS and the kernel's entry paths are assembly ([O.6](sec_ai_o#o6)).
...
- [ ] DBG-05 — **The same adapter against hardware, through the Debug Agent.** Only once [R.24](sec_ai_r#r24)'s condition holds — the command set has stopped moving — and the host transport of [Q191](sec_ai_q#q191) is decided.
