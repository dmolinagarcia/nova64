...
- EM4.3 — **The opcode map covers loads, stores, the ALU group, read-modify-write, branches, jumps and subroutines, stack, transfers, flag and mode control, `BRK`, `COP`, `WDM` and `STP`**, across direct page and its indexed forms, absolute and indexed, long and long indexed, the indirect forms including `[dp]` and `[dp],Y`, and stack-relative. **`MVN` and `MVP` are not implemented**, which is why [EM6.7](sec_ai_em6#em67) is arithmetic rather than measurement.
  NOTE: **`WDM` being in that list is necessary and not sufficient — it has to be a two-byte no-op, and nothing else** ([VM2.24](sec_ai_vm2#vm224)). Several 65816 emulators use it as a debugger breakpoint, and this one halts on anything unimplemented ([EM4.4](sec_ai_em4#em44)); **either behaviour makes the BIOS CPU probe report an Argon 2 that does not exist**, because the probe's whole premise is that a real part leaves A untouched ([I.7](sec_ai_i#i7), [D89](sec_ai_q#d89)). Two bytes consumed, A, P and the cycle count unchanged.
  TEST: Execute `wdm #$00` with A non-zero and confirm A, P and the cycle count are unchanged and the PC advanced by two.
...
- EM4.5 — [[!blocking]] **The core is not validated, and a program that runs proves nothing.** The external suites are the only third-party oracle in this entire project and they have not been wired in ([EM7.23](sec_ai_em7#em723)). Nothing measured by the emulator should be trusted more than the core underneath it.
  NOTE: **An outside example of the same claim**: a TypeScript 65816 core that ran a multitasking OS for years subtracted wrongly, took `BRK` through the wrong address and native interrupts through the emulation vector ([EM8.19](sec_ai_em8#em819)).
...
- EM4.16 — **Register width is not tracked for you, and getting it wrong assembles cleanly and executes garbage.** `ca65` needs `.a8`, `.a16`, `.i8` and `.i16` to know how to assemble immediate operands, and they must match what `REP` and `SEP` actually did at run time. **It is the single most common way to lose an afternoon on this architecture** ([EM7.13](sec_ai_em7#em713)).
  ...
  NOTE: **Smart mode narrows the first trap without closing it.** With `.smart +`, ca65 follows `REP` and `SEP` with constant operands and sets the widths itself — in straight-line code only, since it cannot follow a branch, a `JSR`, or a routine entered from callers that disagree. [O.10](sec_ai_o#o10) adds the other two halves: macros that emit the instruction and the directive together, and an entry width written on every routine ([D114](sec_ai_q#d114)).
