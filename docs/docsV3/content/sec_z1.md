...
| Term | Meaning |
|---|---|
| EC | Embedded Controller — the always-on microcontroller handling power, boot and housekeeping. Here, the RP2354B in QFN-80. It stays off the shared bus entirely, and its in-package 2 MB flash is the board's only mandatory non-volatile storage ([D.8](sec_d#d8)). |
...
| USB host controller | The EC's native USB, assigned to host mode in hardware — it handles hub enumeration, the `PRE` packets Low Speed devices need, and bulk transfers in silicon. The assignment is one-way: there is no device port and therefore no USB recovery path ([D.9](sec_d#d9)). |
| `PRE` packet | The Full Speed preamble a host must emit before every transaction with a Low Speed device behind a hub. Historically the most fragile path in this family's host stack, which is why [Q102](sec_q#q102) is a bring-up gate rather than an assumption. |
| Hub cascade | A second hub level, created whenever a user plugs a hub into a downstream port. The on-board hub already consumes the one level guaranteed to work ([D.10](sec_d#d10)). |
| `J-DBG` · `J-QSPI` | The EC's two headers. The first carries SWD and the console UART, serviced by one probe over one cable; the second carries the bootrom UART and direct access to the flash die ([D.12](sec_d#d12)). |
| Rescue debug port | An access point in the EC's debug architecture that can reset the chip even when its firmware is hostile to debugging — a tight loop, a routine that reconfigures the debug pins, a crashed power sequencer. The reason SWD is the primary path rather than the convenient one ([D.13](sec_d#d13)). |
| Bootrom UART boot | The EC's second recovery tier: two straps at reset put the bootrom on a 1 Mbaud UART carried on flash-interface pins. **It writes SRAM only**, which is why the RAM flash-writer stub exists ([D.14](sec_d#d14)). |
...
| Monitor | Minimal interactive debugger: examine and alter memory, load over serial. Two of them exist — one in the BIOS, running on the 65816, and the EC console of [sheet R](sec_r), which covers the same ground from outside the CPU and is available a whole stage earlier. |
...
| FAT32 | The filesystem this document once put on the boot card, and now the *first* of the three the host track implements ([P.26](sec_p#p26)) — chosen there for being the cheapest way to get a real filesystem mounted, not for being used here. The boot card is NVFS ([G.3](sec_g#g3)). |
...
## Filesystems
  NOTE: → [sheet Y1](sec_y1) · [sheet Y2](sec_y2) · [sheet Y3](sec_y3)

| Term | Meaning |
|---|---|
| VFS | Virtual File System — the layer between the syscall table and the filesystem drivers. It owns path resolution, the mount table, vnode identity and the error space, because none of those can be decided inside a driver that cannot see across a mount boundary ([Y1.1](sec_y1#y11)). |
| vnode | The in-memory object for one *in-use file*, cached and reference-counted, identified by its mount and a stable file identifier. Not the same as an open file: two `open()` calls share a vnode and hold separate offsets ([Y1.5](sec_y1#y15)). |
...
| Superblock | The block that describes the whole volume: sizes, region locations, counters, feature flags. NVFS keeps a byte-identical backup at the last block, written only by `mkfs` and `nvfsck`, and **never silently falls back to it** ([Y2.6](sec_y2#y26)). |
| Bitmap · summary | One bit per block or inode saying whether it is allocated, plus — in NVFS — a free-count per bitmap block so the allocator can skip a full one without reading it ([Y2.7](sec_y2#y27)). |
| Extent | A run of consecutive blocks described by one entry: first logical block, first physical block, length. Replaces the indirect tree for files laid down in one or a few runs, which is most of them ([Y2.10](sec_y2#y210)). |
| Indirect block | ext2's alternative: a block full of block pointers, with double and triple levels above it. Cheap to implement, expensive to read — each level is another transfer before any data moves ([Y3.5](sec_y3#y35)). |
| Hole | A logical block covered by no extent and no pointer. Reads as zeros and occupies nothing. |
| Journal | A log of metadata changes written before the changes reach their home locations, so that recovery is a forward scan and a sequence of block copies. NVFS's is **physical, redo-only, metadata-only and circular**, which is what makes replay idempotent ([Y2.15](sec_y2#y215)). |
...
| Checkpoint | Writing a committed transaction's blocks to their home locations and advancing the journal's start pointer. May be deferred and batched; **advancing it before those writes are durable is the one way to lose data** ([Y2.16](sec_y2#y216)). |
| Barrier | The point at which every write issued before it is durable before any issued after. On SD that means waiting out the busy state and confirming — an assumption rather than a guarantee ([Q96](sec_q#q96)). |
| `data=ordered` | The rule that a transaction's file data reaches the medium before its commit block, so metadata can never reference a block whose contents are stale. The faster alternative is refused here ([Y2.18](sec_y2#y218)). |
| Orphan | An inode whose link count reached zero while it was still open. Threaded onto a list in the superblock and freed at the next mount, which is what stops a crash from leaking space ([Y2.19](sec_y2#y219)). |
| Deferred delete | Unlinking a file that is still open: the name goes, the blocks are released when the last reference does. Makes deleting an open file safe rather than catastrophic ([Y1.18](sec_y1#y118)). |
| Feature flags | Three masks in the superblock. An unknown *incompatible* bit refuses the mount, an unknown *read-only* bit mounts read-only, an unknown *compatible* bit is ignored — and the implementation **must name the bit it did not recognise** ([Y2.20](sec_y2#y220), [Y3.4](sec_y3#y34)). |
| htree · `DIR_INDEX` | ext2's hashed directory index, hidden inside fake entries so that a linear reader sees the directory correctly. **Reading is safe and writing is not**: an insert that does not update the index leaves a file that exists, occupies space, appears in `readdir` and cannot be opened by name ([Y3.6](sec_y3#y36)). |
| `readdir` cookie | An opaque value a driver hands back to resume iteration. NVFS advances a slot index and ext2 a byte offset; neither is expressible in the other's terms, so the VFS stores it and never interprets it ([Y1.3](sec_y1#y13)). |
| `fsck` · `e2fsck` · `nvfsck` | Host-side repair tools. **There is no on-target `fsck` and none is planned** for either filesystem: NVFS recovers from its journal and its orphan list, and ext2's recovery is the host's ([Y2.22](sec_y2#y222), [Y3.2](sec_y3#y32)). |
| Differential testing | Performing every operation twice — once through the implementation under test and once through a reference driver — and comparing. What makes the host track tractable ([Y3.15](sec_y3#y315)). |
| Crash injection | Hooking a specific write number, truncating the image there, and checking that what remains is clean or trivially recoverable. The harness that found draft 2's ordering bugs ([Y2.2](sec_y2#y22)). |
| FUSE | Filesystem in Userspace — the Linux mechanism the host track mounts through. Its **low-level, inode-keyed API** is the one that transfers; the path-based one does not, because a driver here never sees a path ([P.28](sec_p#p28)). |
| MBR type `0x7F` | The partition type byte reserved for local and experimental use, and the one an NVFS volume carries. `0x7A` and `0x7B` are unsafe — they alias hidden-FAT types ([D52](sec_q#d52)). |
...
| DAPLink | The on-board debug probe of the i9 extension board, giving JTAG plus a serial bridge. Absent on a carrier that hosts the SODIMM socket directly, which is why the RP2354B picks up ECP5 JTAG there ([P.10](sec_p#p10)). |
