...
- T.52 — **Backlight `EN` and PWM are not on this list, and that is a decision.** REV A gave them to Neon and put `BACKLIGHT_EN` in `NEON_CTRL`; they belong to the EC, which already owns every rail enable through its expander ([D.6](sec_ai_d#d6)) and is the only party awake both before Neon is configured and while the switched domain is coming down. Neon recovers two pins it needs and the bit leaves the register map. **This closes [Q6](sec_ai_q#q6)** (→ [D33](sec_ai_q#d33)).
