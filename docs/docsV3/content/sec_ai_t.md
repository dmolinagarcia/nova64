...
- T.2 — **The CPU must not draw.** The 65816 at 8 MHz moves ~1.3 MB/s through the `$FE` aperture, ~1.6 MB/s at best; a full 320×200 screen costs it ~40 ms, more than two frames. The drawing engine does the same fill in **0.31 ms**. Every decision below moves work off the CPU, ending in the autonomous command lists of [T.39](sec_t#t39), where per-frame cost is ~180 bytes of stores.
...
- T.4 — **The reference clock is 25 MHz, not 12.** REV A derived a single 102.000 MHz domain from a 12 MHz crystal. This board has **one 25 MHz oscillator** starred to all three FPGAs ([B.7](sec_b#b7)), and 102.000 MHz is not reachable from it: the iCE40 PLL needs ≥10 MHz at its phase detector, allowing only `DIVR=0` (25 MHz) or `DIVR=1` (12.5 MHz), and neither lands on 102. Two settings bracket it.
...
- T.6 — **Neon's SDRAM is 64 MB, not 32.** REV A assumed a W9825G6KH; [D14](sec_q#d14) put an AS4C32M16SB-7TIN on both FPGAs, with the W9825G6KH a drop-in fallback on the same footprint. Bandwidth is unaffected — 16 bits either way — and every capacity argument gets easier: the pre-rendered level buffer of [T.60](sec_t#t60) is 1.64 MB, **2.5 % of memory rather than 5 %**, and a dozen levels could be resident at once.
...
- T.9 — **The second PLL is free and has two claimants.** REV A spent both, one on pixels and one on SDRAM; the single-domain decision returns one. Audio wants a master clock the I2S divider can hit cleanly, and the VGA bring-up path of [sheet H](sec_h) wants 25.175 MHz for 640×480 — which the video domain cannot supply, since 103.125 / 4 is 25.78 MHz, 2.4 % fast. Most monitors tolerate that; the panel is the product and VGA is scaffolding, so this is not worth a fight (→ [Q54](sec_q#q54)).
...
- T.10 — [[!blocking]] **103 MHz is demanding for an iCE40 HX and closure is unproven until synthesis.** SDRAM controllers in this range are documented on the family; that is not the same as having one placed and routed with a blitter beside it. **Fallback:** run everything single-rate at 51.5625 MHz, halving peak SDRAM bandwidth to ~103 MB/s. Every performance figure below halves and still beats CPU-driven rendering by more than an order of magnitude. **The architecture does not depend on 103 MHz; only the margins do** (→ [Q44](sec_q#q44)).
  NOTE: The fallback keeps the property [T.8](sec_t#t8) was bought for. At a single rate the clock enable simply disappears and pixel and memory stages run together, so there is still no crossing anywhere inside Neon — what halves is bandwidth, not the architecture.
...
- T.11 — **EBR holds everything that must be reachable without a wait state, and everything scanout needs at single-cycle latency.** It is genuinely dual-port: port A serves scanout, port B serves the CPU aperture. At 103 MHz fabric against 8 MHz PHI2 every CPU access to EBR — read *or* write — completes inside its bus cycle. No write FIFO, no shadow copy in system SRAM, no arbiter, and no wait line asserted. Glyph fetch is one cycle; in SDRAM the same random access would pay a row activation (~60 ns) for no benefit.
  NOTE: The slack is 103.125 MHz against 8 MHz PHI2 — about 12.9 core cycles per bus cycle — which is what lets "no wait line" be a statement rather than a hope. Even at [T.10](sec_t#t10)'s halved rate it is still six, so the fallback does not put this claim at risk.
...
| 4 | **The software model stays write-only** | The restriction was doing useful work: it is *why* save-under is a blitter copy and why assets flow one way. With reads available someone will use them, and the symptom is a slow GUI rather than a compile error ([T.16](sec_t#t16)) |
  ...
  NOTE: The same wire serves a second requester with no clock involved. When **Helium** reads VRAM — the debug agent issuing an external cycle with `BE` low and the CPU off the bus ([R.5](sec_r#r5)) — there is no PHI2 to stall; Helium waits for busy to clear before sampling D0–D7. **`NEON_BUS_BSY` is a ready line that Helium happens to use to gate PHI2 when the requester is the CPU**, which is the cleanest way to hold the mechanism in mind.
...
- T.50 — That leaves ~1,800 LUT of margin. With the tile and sprite engines it came to ~7,300 — 95 %, not routable once audio grows; the removal of [T.44](sec_t#t44) is the whole difference. **Mode 0 alone is ~900 LUT, about 12 % of the device**, which is the number that matters for [T.53](sec_t#t53): the console is cheap enough to synthesise on its own, long before anything else in Neon exists.
...
- T.52 — **Backlight `EN` and PWM are not on this list, and that is a decision.** REV A gave them to Neon and put `BACKLIGHT_EN` in `NEON_CTRL`; they belong to the EC, which already owns every rail enable through its expander ([D.6](sec_d#d6)) and is the only party awake both before Neon is configured and while the switched domain is coming down. Neon recovers two pins it needs and the bit leaves the register map. **This closes [Q6](sec_q#q6)** (→ [D33](sec_q#d33)).
...
- T.54 — **The BIOS inherits a working display and initialises nothing** — no probe, no mode, no font: it stores bytes to `$FE:0000` and characters appear. It reads `NEON_ID` once for `$4E`, to tell a fitted Neon from an absent one, continuing either way. `putchar` is a 16-bit `STA` to `$FE0000,x` — **absolute-long indexed: the bank byte is in the instruction, so it runs from any bank** — then a 16-bit store to `CURSOR_POS`, latching on its high byte. Scrolling is [T.27](sec_t#t27)'s pointer move.
- T.55 — **The kernel's mechanism is identical to the BIOS's** — same stores, same addresses — and the four differences are all layering: the console becomes a kernel driver behind [J.4](sec_j#j4)'s five-function interface, as a device node; bank `$FE` stays privileged where it matters ([T.21](sec_t#t21)); a custom font can be loaded into `$FE:2000` in vblank; and the display tick can come from the vblank IRQ.
  NOTE: **One constraint must be built in from the start.** Graphics modes may reclaim the text buffer's EBR ([T.12](sec_t#t12)), so a mode switch and back can destroy the screen: **the console driver must hold its own scrollback and repaint from it** (→ [Q47](sec_q#q47)).
...
- T.63 — **Each stage is a usable machine, and if work stops at any of them nothing is stranded.** It is the layering of [T.37](sec_t#t37)–[T.39](sec_t#t39) again: a failure in the command processor still leaves a GUI-capable blitter, a failure in SDRAM a working boot console. N3 delivers the primary milestone and N5 the games capability, **in that order and separably**.
