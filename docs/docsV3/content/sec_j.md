...
- J.5 — Framebuffer: the preferred path is an ioctl that maps VRAM pages (`$FE`) into the process's address space — no per-pixel `write()`. Battery and power are the mirror case: the logic lives on the RP2040 and the kernel sees an ordinary I2C-translating driver, with no special case in the ABI.
