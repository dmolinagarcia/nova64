...
- I.2 — At reset it provides: CPU init (native mode `CLC/XCE`, stack, direct page), MMU in supervisor identity mapping, cache init, quick memory test, UART console, and boot screen.
  ...
  NOTE: "Quick" is doing real work in that list: the BIOS is already fetching its own instructions out of the memory it would be testing, so it can only check what is not under it. The march test that has no such problem runs at [E2](sec_ai_p#e2), before any CPU exists ([R.22](sec_ai_r#r22)).
