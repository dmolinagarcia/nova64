...
- W.38 — `BCK`, `LRCK`, `DIN`. [Sheet T](sec_t)'s pin budget lists **4** for I2S with "three if `SCK` is grounded" beside it, and [sheet H](sec_h) grounds `SCK`; the condition is met and has been since the strapping was written down, so **Neon's total is 99 of ~107, not 100**. `XSMT` is not a fourth pin — it is strapped to 3V3 through the RC of [W.7](sec_w#w7), and `AUD_CTRL.MUTE` drives it only if [W.40](sec_w#w40)'s recommendation is taken, which costs a pin and buys the mute.
