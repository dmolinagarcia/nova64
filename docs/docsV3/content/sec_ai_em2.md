...
- EM2.14 — **Four steps, and none of them is emulated hardware.** A host configuration file selects an SD image, a boot image and a parameter set; the boot image is loaded at the address the EC's `MEM_WRITE` sequence would have placed it; the Helium and Neon models are initialised to their post-configuration reset state; CPU reset is released and execution begins at the reset vector. No SPI configuration traffic and no power sequencing exists in the model at all.
