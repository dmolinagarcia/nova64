...
- A.7 — Scope of this document: an index + outline tying all the pieces together. Detail lives in the sheets: `hoja-1-3-config-fpga.md` … `hoja-1-6-conectores.md`.
  NOTE: **Power is the exception, and the direction of travel.** Sheets [C](sec_c) and [S](sec_s) are no longer an outline of a document held elsewhere — they *are* the power specification at REV C, and the REV A design they grew out of is superseded rather than summarised. Everything living hangs here from now on; the remaining external sheets are the ones that have not yet been folded in.
